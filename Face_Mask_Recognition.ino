...
bool face_match_found = false;


const char* ssid     = "Mi 10T";   //your network SSID
const char* password = "jaakemuhdekho";   //your network password

const char* apssid = "ESP32-CAM";
const char* appassword = "12345678";

String filepath[15] = {"/1.jpg", "/2.jpg", "/3.jpg", "/4.jpg", "/5.jpg", "/6.jpg", "/7.jpg", "/8.jpg", "/9.jpg", "/10.jpg", "/11.jpg", "/12.jpg", "/13.jpg", "/14.jpg", "/15.jpg"};  //1.jpg, 2.jpg, ...., 35.jpg
int image_width = 400;
int image_height = 296;
String recognize_face_matched_name[7] = {"Master Sarvagya", "Mansi", "Pranjal", "Name3", "Name4", "Name5", "Name6"};
...
void FaceMatched(int faceid) {  //辨識到註冊人臉執行指令控制
  ...
  lcd.print("Welcome, ");
  lcd.setCursor(0, 3);
  lcd.print(recognize_face_matched_name[faceid]);
...
int Mask_read_times = 0;

void loop() {

  if (Mask_read_times == 0 && P1 != "") {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Scanning for Mask");
  }

  for (int i = 0; i < 10; i++) {
    if (P1 == "Mask")
      Mask_read_times++;
    if (P1 != "Mask" && P1 != "" && Mask_read_times >= 1)
      Mask_read_times--;
    delay(500);
  }

  if (Mask_read_times >= 6) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("Please Remove Mask");
    lcd.setCursor(0, 2);
    lcd.print("for Identification");

    delay(1500);

    int num_face_recog = 30;

    while (num_face_recog && face_match_found == false)
    {
      faceRecognition();
      delay(100);
...
<html>
    <head>
        ...
        <title>Mask Detection & Face Recognition</title>
        <style>
